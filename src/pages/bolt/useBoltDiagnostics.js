...
async function uploadRound(blob, signal) {
...
export function useBoltDiagnostics() {
  ...
  const [upload, setUpload] = useState({ current: 0, peak: 0, avg: 0, lastResult: null });
    ...
    while (!signal.aborted) {
      if (chunkBytesRef.current > 50_000) {
        ...
        try {
          ...
          const result = await uploadRound(blob, signal);
          if (result) {
            uploadResultsRef.current.push(result);
            setUpload((u) => ({
              current: result.upload_mbps,
              peak: Math.max(u.peak, result.upload_mbps),
              avg: average(uploadResultsRef.current.map((r) => r.upload_mbps)),
              lastResult: result,
            }));
          }
        } catch {
  ...
  const finalize = useCallback((controller, total) => {
    ...
    setResourceTiming(timingSummary);
    ...
    const grade = computeGrade({ avgPing, jitter, lossPct });
    ...
    setSummary({
      ...
      uploadAvg: uploadResultsRef.current.length ? average(uploadResultsRef.current.map((r) => r.upload_mbps)) : 0,
      uploadSamples: uploadResultsRef.current.length,
  ...
  const start = useCallback(() => {
    ...
    setUpload({ current: 0, peak: 0, avg: 0, lastResult: null });
    setSummary(null);
    ...
    const onOffline = () => setInterrupted(true);
